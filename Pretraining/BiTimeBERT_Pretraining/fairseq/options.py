...
from fairseq.models import ARCH_MODEL_REGISTRY, ARCH_CONFIG_REGISTRY, PRETRAINED_MODEL_ARCHIVE_MAP
...
def get_parser(desc, default_task='translation'):
    ...
    parser.add_argument('--multi-train', action='store_true', help='disable progress bar')
...
def add_dataset_args(parser, train=False, gen=False):
    ...
    group.add_argument('--docid2timestamp-dir', default=None, type=str)
    group.add_argument('--temp_granularity', default='Year', choices=['Year', 'Month', 'Week', 'Day'])
...
def add_optimization_args(parser):
    ...
    group.add_argument('--bert-model', default=None, choices=PRETRAINED_MODEL_ARCHIVE_MAP.keys())    
    group.add_argument('--add-external-temp-tokens', action='store_true')
    
    group.add_argument("--max-sentences", "--batch-size", default=50, type=int)
    group.add_argument('--lr', "--learning-rate", default=3e-5, type=float)
    group.add_argument("--num-train-epochs", default=2.0, type=float)
    group.add_argument("--warmup-proportion", default=0.01, type=float, help="Proportion of training to perform linear learning rate warmup for. E.g., 0.1 = 10%% of training.")
    group.add_argument('--gradient-accumulation-steps', type=int, default=1, help="Number of updates steps to accumualte before performing a backward/update pass.")
...
def add_model_args(parser):
    ...
    group.add_argument(
        '--arch', '-a', default='cased_bert_hf',help='Model Architecture',
    )
